...
from treasuremap import layout_treasuremap
...
class TreasuremapTests(unittest.TestCase):
    def testUMAP(self):
        ...
        g = Graph(edges)
        lo = layout_treasuremap(g, dist=dist, epochs=500, sampling_prob=0.3)
        self.assertTrue(isinstance(lo, Layout))
        ...
        lo_adj = layout_treasuremap(g, dist=dist, epochs=1, sampling_prob=1, seed=lo)
        self.assertTrue(isinstance(lo_adj, Layout))

        # Same but inputting the coordinates
        lo_adj = layout_treasuremap(g, dist=dist, epochs=1, sampling_prob=1, seed=lo.coords)
        self.assertTrue(isinstance(lo_adj, Layout))
